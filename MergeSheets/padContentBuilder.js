...
function buildSessionForPracticeSpaceContent(row) {
	...
	padContent += buildContentSection("Need Financial Aid?", row['willpresentersneedfinancialassistancetoattendandpresentattheamc']);
	var financialAssistanceRequest = row['willpresentersneedfinancialassistancetoattendandpresentattheamc']
	if(financialAssistanceRequest === 'Yes') {
		padContent += buildContentSection("If so, how many people?", row['ifyouansweredyesabovepleaselistthenumberof25scholarshipcouponsyouarerequesting._2']);
	}
